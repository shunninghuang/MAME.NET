...
namespace mame
{
    public class OKI6295
    {
        ...
        public byte[] okirom;        
        public okim6295Struct OKI;
        public static OKI6295[] oo1 = new OKI6295[2];
        public adpcm_state[] adpcm;
        private int[] index_shift = new int[8] { -1, -1, -1, -1, 2, 4, 6, 8 };
        private int[] diff_lookup = new int[49 * 16];
        private uint[] volume_table = new uint[16];
        private int tables_computed = 0;
        ...
        private void reset_adpcm(int i)
        {
        ...
        private short clock_adpcm(int i, byte nibble)
        {
        ...
        private void generate_adpcm(int i, short[] buffer, int samples)
        {
            int i1 = 0;
            if (OKI.voice[i].playing)
            {
                ...
                while (samples != 0)
                {
                    ...
                    buffer[i1] = (short)((uint)clock_adpcm(i, (byte)nibble) * OKI.voice[i].volume / 256);
                    i1++;
                    samples--;
                    if (++sample >= count)
                    {
        ...
        public void okim6295_update(int offset, int length)
        {
            int i;
            for (i = 0; i < length; i++)
            {
                OKI.stream.streamoutput[0][offset + i] = 0;
            }
            for (i = 0; i < 4; i++)
            {
                ...
                while (remaining != 0)
                {
                    ...
                    {
                        OKI.stream.streamoutput[0][offset + samp] += sample_data[samp];
                    }
                    remaining -= samples1;
                }
            }
        }
        public void okim6295_start()
        {
            int voice;
            compute_tables();
            OKI.command = -1;
        ...
        public void okim6295_reset()
        {
            int i;
            OKI.stream.stream_update();
            for (i = 0; i < 4; i++)
            {
                OKI.voice[i].playing = false;
            }
        }
        public void okim6295_set_bank_base(int base1)
        {
            OKI.stream.stream_update();
            OKI.bank_offset = base1;
        }
        public void okim6295_set_pin7(int pin7)
        {
            int divisor = pin7 != 0 ? 132 : 165;
            //stream_set_sample_rate(info->stream, info->master_clock / divisor);
        }
        public int okim6295_status_r()
        {
            int i, result;
            result = 0xf0;
            OKI.stream.stream_update();
        ...
        public void okim6295_data_w(int data)
        {
            if (OKI.command != -1)
            {
